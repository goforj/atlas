...
while IFS= read -r -d '' module; do
  ...
  (
    cd "${module_dir}"
    GOWORK=off go run github.com/CycloneDX/cyclonedx-gomod/cmd/cyclonedx-gomod@v1.12.0 mod -json -type library -test -output "${output}" .
  ) < /dev/null
...
done < <(find . -type f -name package-lock.json -not -path './node_modules/*' -print0)
