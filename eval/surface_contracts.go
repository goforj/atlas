...
func promotedSurfaceContracts() []surfaceContract {
	return []surfaceContract{
		...
		},
		{
			...
			allowedChanges:      generatedEnvironmentChanges("go.mod", "internal/storages/*_gen.go", "internal/storages/README.md", "internal/invoices/*attachment*.go", "app/wire/inject_services_app.go", "app/wire/app.go"),
		...
		},
		{
			...
			allowedChanges:  []string{".env.local", "internal/inspects/README.md"},
		...
		},
		{
			...
			qualityTestPatterns: []string{"internal/invoices/*_test.go", "app/wire/*_test.go"},
			sources: []sourceContract{
		...
		},
		{
			...
			sources: []sourceContract{
				...
				{id: "event-reaction", paths: []string{"internal/events/*.go", "internal/notifications/*.go", "app/lifecycle.go"}, identifiers: []string{"Subscribers", "UserCreatedHandler", "Register", "Startup", "Shutdown"}, forbiddenCalls: []string{"TODO"}, declarations: []declarationContract{{name: "Register", receiver: "Subscribers", identifiers: []string{"UserID"}, selectorCalls: []string{"Subscribe"}, selectorCallChoices: [][]string{{"Handle", "HandleUserCreated"}}, forbiddenCalls: []string{"Background"}}, {name: "Startup", receiver: "LifecycleRegistry", selectorCalls: []string{"Register"}, forbiddenCalls: []string{"Background"}}, {name: "Shutdown", receiver: "LifecycleRegistry", selectorCalls: []string{"Close"}, forbiddenCalls: []string{"Background"}}}},
				{id: "user-event-publisher", paths: []string{"internal/users/*.go", "internal/events/*.go"}, identifiers: []string{"UserEventPublisher", "NewUserEventPublisher"}, forbiddenCalls: []string{"TODO"}, declarations: []declarationContract{{anyName: true, receiver: "UserEventPublisher", identifiers: []string{"ctx", "UserID"}, selectorCalls: []string{"Publish", "WithContext"}, forbiddenCalls: []string{"Background"}}}},
			},
