...
func enabledSurfacePaths(root string, agent agents.Agent, features config.Features, paths []string, guidelinesOwned bool) []string {
	...
	for _, path := range paths {
		...
		case path == guidelinePath:
			if guidelinesOwned && features.Guidelines {
